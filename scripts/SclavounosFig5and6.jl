...
let
    ...
    wing = LiftingLineTheory.make_elliptic(StraightAnalyticWing, 4, 4)
    ...
    figure()
    ax = gca()
    ax.imshow(imread("scripts/Sclav87ref/Fig5.PNG"), extent=[0, 8, 0, 8])
    ax.set_aspect("auto")
    ...
    figure()
    ax = gca()
    ax.imshow(imread("scripts/Sclav87ref/Fig6.PNG"), extent=[0, 8, -270, 0])
    ax.set_aspect("auto")
