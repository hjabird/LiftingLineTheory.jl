...
let
    ...
    dt = 0.05
    ninner = 8
    kinem_file = "smoothed_fast_heave_eld_0p05U_0.dat"
    casename = "AR"*string(AR)*"Rect_hSmoothFast_Ni"*string(ninner)*"_dt"*string(dt)
    ...
    function openfoam_kinem_to_rigidkinem(kinem_mat_3d, pivot_pos)
        local k2d = zeros(size(kinem_mat_3d)[1], 6)
        k2d[:, 1] =  kinem_mat_3d[:, 1]
        k2d[:, 2] =  kinem_mat_3d[:, 3]
        k2d[:, 4] =  kinem_mat_3d[:, 7]
        k2d[:, 6] .=  pivot_pos
    ...
    kinem, stime, etime = openfoam_kinem_to_rigidkinem(read_openfoam_kinematics(kinem_file), -0.5)
